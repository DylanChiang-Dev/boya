...
1. **框架從文獻長出，不從記憶編。** 候選框架要能對回研究者手上已有的文獻。你可以基於通識**提名**一個庫裡沒有的框架，但必須誠實標「庫存支撐」（✅有／🟡部分／❌需補 X 篇）；沒地基的一律標「需補」、交回研究者去找真文獻（指回 `citation-verify`／`lit-matrix`），**絕不假裝已有支撐**。框架的承重文獻也要是真的。**延伸：✅ 支撐 ≠ 免遷移**——庫存支撐只證框架文獻為真，不證它能無痕套到新對象上。遷移代價有兩種，都必須在「理論代價」欄標明：①**世代／技術遷移**（實測：Davis 1989 的 TAM 為確定性 mainframe IT 建，套有幻覺的生成式 AI，PU/PEOU 接不住）；②**應用場景跨域遷移**（實測：Beauchamp & Childress 1979 四原則來自生物醫學倫理，套 LLM silicon sampling 時「自主原則」預設真實受訪者，直接失去著力點）——兩種都是「有支撐卻不能無痕套用」的隱形坑。
