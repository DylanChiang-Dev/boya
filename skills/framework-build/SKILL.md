...
1. **框架從文獻長出，不從記憶編。** 候選框架要能對回研究者手上已有的文獻。你可以基於通識**提名**一個庫裡沒有的框架，但必須誠實標「庫存支撐」（✅有／🟡部分／❌需補 X 篇）；沒地基的一律標「需補」、交回研究者去找真文獻（指回 `citation-verify`／`lit-matrix`），**絕不假裝已有支撐**。框架的承重文獻也要是真的。**延伸：✅ 支撐 ≠ 免遷移**——庫存支撐只證框架文獻為真，不證它能無痕套到新對象上。老框架套新技術（實測：Davis 1989 的 TAM 是為確定性 mainframe IT 建的，套到有幻覺、不確定性的生成式 AI，PU/PEOU 兩構念接不住），理論代價欄必須標明世代／場景遷移成本，否則是另一種兩張皮。
...
3. **不堆框架沙拉。** 主框架最多一個。每多一個輔助／中介框架，都要回答「它幹的活前面的框架幹不了嗎」；答不出就砍。理論豐富不是靠層數堆出來的。**注意框架沙拉的隱形版：「並列列出」不等於「並列地位」。** 文獻、摘要或關鍵詞常把幾個理論平鋪並列（實測：碩論官方關鍵詞把 TAM／科技壓力／社會影響平鋪，一度被擺成三個並列主框架）——定錨時要追問哪個扣題最直接（選它當唯一主框架），其餘明確降為中介／輔助並說清各補主框架哪一塊盲區，不得順著平鋪排列拉平地位。
4. **框架要對得上研究問題與差異化卡位。** 框架是透鏡不是裝飾。盯兩件事：它解釋的東西扣不扣得回研究問題？選它會不會把研究推回擁擠紅海、丟掉差異化？（實測：選准同盟當主框架，會把「經濟安保產業鏈」滑回擁擠的「日台安全」紅海。）
5. **框架要能往下操作化。** 定錨完要能交給 `method-design` 變指標、交給 `outline-builder` 長章節。一個落不到實證抓手的框架是空的。報告結尾要為每個關鍵概念指出「實證抓手候選」。**混合方法的抓手要「雙棲」**：若研究是量化＋質性混合，每個關鍵概念要同時指出「問卷怎麼測」與「訪談怎麼追」兩種抓手，不得只列可量化指標（實測：碩論一度只給 PU/PEOU 量表，漏了 6 場訪談那一半，框架與質性兩張皮）。實證抓手依方法形態分排，框架才接得住量化與質性兩條腿。
